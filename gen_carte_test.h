...
bool room_is_good(point begin, point end, vector<vector<int>>* Good_for_room){
...
point try_add_path(vector<point> path, vector<vector<int>>* matrix_con, vector<room*>* rooms, vector<vector<int>>* grid) {
...
vector<point> add_to_good_for_room( vector<vector<int>>* Good_for_room, vector<vector<int>>* grid, room& new_room, vector<vector<int>>* matrix_con, vector<room*>* rooms, bool make_path) {
	...
	for (int x = min(new_room.begin.x, new_room.end.x); x < max(new_room.begin.x, new_room.end.x); x++) {
		for (int y = min(new_room.begin.y, new_room.end.y); y < max(new_room.end.y,new_room.begin.y); y++) {
			if (x < 0 || x >= Good_for_room -> size() || y < 0 || y >= (Good_for_room -> at(0)).size()) {
				return vector<point> {point {-1, -1}};
			} else if ((grid->at(x))[y] == 2 || (grid->at(x))[y] == 3 || (grid->at(x))[y] == 4 || (grid->at(x))[y] == 6) {
				return vector<point> {point {-1, -1}};
	...
	for (int x = min(new_room.begin.x, new_room.end.x); x < max(new_room.begin.x, new_room.end.x); x++) {
		for (int y = min(new_room.begin.y, new_room.end.y); y < max(new_room.end.y,new_room.begin.y); y++) {
			...
			if ((grid->at(x))[y] == 5) {
				if (bool_door) {
					(grid->at(x))[y] = 0;
				} else if (x != new_room.begin.x && x != new_room.end.x -1 && y != new_room.begin.y && y != new_room.end.y -1){
					(grid -> at(x))[y] = 0;
				} else {
					(grid -> at(x))[y] = 4;
					//bool_door = true;
	...
	if (make_path) {
		...
		for (int essai_porte = 0; essai_porte < 10; essai_porte++) {
			if (essai_porte != 0){
				print("nouvel essai de porte");
			...
			for (int k = 0; k < 10; k++) {
				...
				print("hello17");
				print(-door.x + (int)(Good_for_room -> size()) - 4);
				print(-door.y + (int)((Good_for_room -> at(0)).size()) - 4 );
				...
				if (dir_x){
					print("hello20");
					len_x = random_int(min(8,-door.x + (int)(Good_for_room -> size()) - 4)) + 4;
					print("hello21");
				} else {
					print("hello22");
					len_x = random_int(min(8, door.x - 4)) + 4;
					print("hello23");
				}
				if (dir_y) { 
					print("hello24");
					len_y = random_int(min(8,-door.y + (int)((Good_for_room -> at(0)).size()) - 4)) + 4;
					print("hello25");
				} else {
					print("hello26");
					len_y = random_int(min(8, door.y - 4)) + 4;
					print("hello17");
				}
				print("hello18");
				...
				if (point_final.x != -1) {
					...
					print("hello15");
					return vector<point> {point_final, point {len_x, len_y}};
				} else {
					print("hello16");
					continue;
				}
...
point make_room (point begin, vector<vector<int>>* Good_for_room, int interdiction_x = -1, int interdiction_y = -1) {
	for (int n_try = 0; n_try < 20; n_try++){
		...
		if (end.x == interdiction_x) {
			end.x++;
		}
		if (end.y == interdiction_y) {
			end.y++;
		}
		end.x += 4 + begin.x;
		end.y += 4 + begin.y;
		...
		if (room_is_good(begin, end, Good_for_room)) {
			return end;
		} else {
			continue;
		}
...
setup create_map(int stage) {
	...
	int n_rooms_min = stage + 1;
	int n_rooms_max = 2*stage + 3;
	vector<vector<int>>	grid = make_matrix(hauteur_grille, longueur_grille);
	vector<vector<int>> Matrix_connection = make_matrix(n_rooms_max, n_rooms_max);
	vector<vector<int>> Good_for_room = make_matrix(hauteur_grille, longueur_grille);
	vector<room*>  rooms;
	for( int i = 0; i < 2; i++) {
	...
	begin.x += stage * 20;
	begin.y += stage * 20;
	int interdiction_x = -1;
	int interdiction_y = -1;
	print("hello1");
	for (int i = 0; i < n_rooms_max; i++) {
		point end = make_room(begin, &Good_for_room, interdiction_x, interdiction_y);
		print("hello2");
		if (end.x == -1){
			return setup {make_matrix(1,1), rooms, false};
		}
		/*if (i != 0){
			cout << "new room" << endl;
		}*/
		room new_room = room {begin, end};
		...
		print("hello3");
		vector<point> test = add_to_good_for_room(&Good_for_room, &grid, new_room, &Matrix_connection, &rooms, make_path);
		print("hello4");
		if (make_path) {
			if ((test[0]).x == -1 && i < n_rooms_min) {
				return setup {make_matrix(1,1), rooms, false}; 
			} else if ((test[0]).x == -1 && i < n_rooms_max) {
				return setup {grid, rooms, true};
			} else {
				begin = test[0];
				print_P(begin);
				begin.x -= 1;
				...
				print(n_rooms_min);
				print(n_rooms_max);
				cout << "begin.x is " << begin.x << " and begin.y is : " << begin.y << endl;
				interdiction_x = test[1].x + 1;
				interdiction_y = test[1].y + 1;				
			}
		}
	}
	return setup {grid, rooms, true};
...
setup setup_map(int stage) {
	setup _setup {make_matrix(1,1), vector<room*> {}, false};
